...
// Define validation rules for project form  ← ADD THIS BLOCK HERE
...
const showProjectDetailsPage = async (req, res, next) => {
    ...
    // Determine if the logged-in user is a volunteer for this project
    let volunteering = false;
    if (req.session.user) {
        volunteering = await isVolunteer(req.session.user.user_id, project.project_id);
    }

    const title = project.title;
    const description = project.description;
    res.render('project', { title, description, project, categories, volunteering });
};

const showNewProjectForm = async (req, res) => {
    const organizations = await getAllOrganizations();
    const title = 'Add New Service Project';
    const description = 'Create a new service project and assign it to an organization.';
...
const showEditProjectForm = async (req, res, next) => {
    ...
    const description = `Edit details for ${projectDetails.title}`;
...
const processEditProjectForm = async (req, res) => {
    // Check for validation errors (reuse projectValidation)
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        errors.array().forEach((error) => {
            req.flash('error', error.msg);
